...
<body>
	<div class="box-positon">
		<div class="rpos">当前位置：文章- 修改</div>
		<form class="ropt">
			<input type="submit" onclick="this.form.action='querytitle.do';"
				value="返回列表" class="return-button" />
		</form>
		<div class="clear"></div>
	</div>
	<div class="body-box" style="float: right">
		<form id="jvForm" action="updatetitle.do?id=${tb.id }"  method="post"  onsubmit="return chkAll()">
			<table cellspacing="1" cellpadding="2" width="100%" border="0"
				...
				<tbody>
					...
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span>文章 ：</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="title" maxlength="100"  id="title"  onblur="chkloginname()"
							value="${tb.title }" />
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="content" maxlength="100"  id="content"  onblur="chkcontent()"
							value="${tb.content}" />
					...
					<tr>
						<tr>
							...
							<td width="80%" class="pn-fcontent"><input type="text"
								class="required" name="author" maxlength="100"  id="cman"  onblur="chkCman()"
								value="${tb.author }" />
								<span id="resultCman"></span></td>
						</tr>
					</tr>
					<tr>
						<tr>
							<td width="20%" class="pn-flabel pn-flabel-h"><span
								class="pn-frequired">*</span>创建时间:</td>
							<td width="80%" class="pn-fcontent"><input type="text"
								class="Wdate" name="crtime" maxlength="100"  onclick="WdatePicker()"
								value="${tb.crtime}" /></td>
						</tr>
					</tr>
				<tr>
						...
						<c:if test="${list.id==tb.channel }">
						<option value="${list.id }"  selected="selected">${list.cname }</option>
						</c:if>
						<c:if test="${list.id != tb.channel}">
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent">
						<c:if test="${tb.isremod==1 }">
								<input type="radio" class="required" name="isremod" value="1"
									maxlength="80" checked="checked" />推荐
						<input type="radio" class="required" name="isremod" value="2"
									maxlength="80" />不推荐
						</c:if>
						 <c:if test="${tb.isremod==2}">
								<input type="radio" class="required" name="isremod" value="1"
									maxlength="80" />推荐
						<input type="radio" class="required" name="isremod" value="2"
									maxlength="80" checked="checked" />不推荐
						</c:if></td>
					</tr>
						<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">是否热度:</td>
						<td width="80%" class="pn-fcontent">
						<c:if test="${tb.ishot==1 }">
								<input type="radio" class="required" name="ishot" value="1"
									maxlength="80" checked="checked" />是
						<input type="radio" class="required" name="ishot" value="2"
									maxlength="80" />否
						</c:if>
						 <c:if test="${tb.ishot==2}">
								<input type="radio" class="required" name="ishot" value="1"
									maxlength="80" />是
						<input type="radio" class="required" name="ishot" value="2"
